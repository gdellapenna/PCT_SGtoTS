...
public class SpatialToString {
    ...
    public CFGGrammar apply(SGGrammar sg, Map<Nonterminal, List<Relation>> relPrecedeResult) throws GrammarException {
        ...
        for (SGProduction p : productions) {
            ...
            if (!A.equals(sg.getStartSymbol())) {
                ...
                for (Relation REL : relprecede_A) {
                    ...
                    CFGProduction newprod = new CFGProduction(new_LHS, anchor, new_RHS);
        ...
        for (SGProduction p : sg.getProductionsByNonterminal(S)) {
            ...
            List<Integer> anchor = p.getAnchor();
            CFGProduction newprod = new CFGProduction(newLHS, anchor, newRHS);
            g.addProduction(newprod);
    ...
    private void removeUselessProductions(CFGGrammar g) throws GrammarException {
        ...
        for (Nonterminal A : useless_symbols) {
            List<CFGProduction> useless_productions = (List<CFGProduction>) g.getProductionsByNonterminal(A);
            Iterator<CFGProduction> it = useless_productions.iterator();
            while (it.hasNext()) {
                it.next();
                it.remove();
            }
